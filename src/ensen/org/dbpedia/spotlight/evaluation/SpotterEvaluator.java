...
package ensen.org.dbpedia.spotlight.evaluation;
...
import ensen.org.dbpedia.spotlight.spot.cooccurrence.training.AnnotatedDataset;
import ensen.org.dbpedia.spotlight.spot.cooccurrence.training.AnnotatedSurfaceFormOccurrence;
...
public class SpotterEvaluator {
	...
	public static void main(String[] args) throws IOException, ConfigurationException, JSONException, InitializationException {
		...
		//
		AnnotatedDataset evaluationCorpus = new AnnotatedDataset(new File("/Users/jodaiber/Desktop/csaw"), AnnotatedDataset.Format.CSAW, spotlightFactory);
		 ...
		 * Advanced Spotter:
		 */
		Spotter spotterWithSelector = SpotterWithSelector.getInstance(spotter, new CoOccurrenceBasedSelector(configuration.getSpotterConfiguration()), spotlightFactory.taggedTokenProvider());
		 ...
		 * At least one noun:
		 */
		spotterWithSelector = SpotterWithSelector.getInstance(spotter, new AtLeastOneNounSelector(), spotlightFactory.taggedTokenProvider());
		...
		spotterWithSelector = SpotterWithSelector.getInstance(spotter, new RandomSelector(spotterBaseResult.valid, spotterBaseResult.common));
	...
	private static class SelectorResult {
		...
		public String toString() {
			return "SelectorResult[" + "valid=" + valid + ", common=" + common + ", part=" + part + "] with Spotter " + this.name;
		}

		public void printResult(SelectorResult baseResult) {

			System.out.println("\n\n\n\nResult for Spotter '" + name() + "' compared with '" + baseResult.name() + "'");
			System.out.println("\nSpotting took " + time + "ms, " + String.format("%1.2f", time / (float) (baseResult.part + baseResult.valid + baseResult.common)) + "ms per spot");

			System.out.println(" part:" + part + " (" + String.format("%1.2f", (((part / baseResult.getPart())) * 100)) + "%)" + ", common:" + common + " (" + String.format("%1.2f", (((common / baseResult.getCommon())) * 100)) + "%)" + ", valid:"
