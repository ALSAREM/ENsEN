...
package ensen.org.dbpedia.spotlight.evaluation.external;
...
public class OntosClient extends AnnotationClient {
	...
	public String annotate(Text text) throws AnnotationException {
		...
		String url = "http://news.ontos.com/api/miner;jsessionid=" + this.authToken + "?query=" + URLEncoder.encode("{" + "\"get\":\"process\"," + "\"ontology\":\"common.english\"," + "\"format\":\"NTRIPLES\"," + "\"text\":\"" + clean(text) + "\"" + "}");
	...
	private Vector<String> getEntities(JSONObject json) throws AnnotationException {
		...
		for (int i = 0; i < triples.length(); i++) {
			try {
				...
				// As advised by Alex
				if (((ONTOS_COMMON_ENGLISH_TYPE_INSTANCEOF).equals(predicate) && (ONTOS_COMMON_ENGLISH_TYPE_PERSON).equals(object)) || ((ONTOS_COMMON_ENGLISH_TYPE_INSTANCEOF).equals(predicate) && (ONTOS_COMMON_ENGLISH_TYPE_ORGANIZATION).equals(object))) {
					entities.add(subject);
				}
