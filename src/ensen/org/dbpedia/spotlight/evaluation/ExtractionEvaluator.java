...
public class ExtractionEvaluator {
	...
	public static void main(String[] args) throws IOException, ConfigurationException, JSONException, InputException, CacheException {
		...
		AnnotatedDataset evaluationCorpus = new AnnotatedDataset(new File(args[0]), AnnotatedDataset.Format.CSAW, spotlightFactory);

		int tp = 0, fp = 0, fn = 0;

		Set<DBpediaResourceOccurrence> goldAnnotations = evaluationCorpus.toDBpediaResourceOccurrences();
		writeAnnotations(spotlightFactory, evaluationCorpus, new File(args[1]));

		/**
		 * Read all annotations made by a configuration of DBpedia Spotlight.
		 */
		List<List<DBpediaResourceOccurrence>> allAnnotatedTexts = readAnnotations(new File(args[1]), evaluationCorpus);
	...
	private static List<List<DBpediaResourceOccurrence>> readAnnotations(File folder, AnnotatedDataset evaluationCorpus) {
		...
		List<List<DBpediaResourceOccurrence>> allAnnotatedOccurrences = new LinkedList<List<DBpediaResourceOccurrence>>();
		...
		for (File file : files) {
			...
			try {
				...
				while ((line = csvReader.readNext()) != null) {
					...
					DBpediaResourceOccurrence dbpediaResourceOccurrence = new DBpediaResourceOccurrence(dbpediaResource, surfaceForm, text, offset);
	...
	private static void writeAnnotations(SpotlightFactory spotlightFactory, AnnotatedDataset evaluationCorpus, File annotationsFolder) throws IOException {
